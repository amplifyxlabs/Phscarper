...
scp -i ~/Downloads/phscraper_key.pem cron-scraper.js ecosystem.config.js google-sheets-exporter.js .env azureuser@20.193.130.170:/home/azureuser/Phscarper/

# Check if we need to upload Google credentials
if [ -f "./google-credentials.json" ]; then
  echo "Copying Google credentials to VM..."
  scp -i ~/Downloads/phscraper_key.pem google-credentials.json azureuser@20.193.130.170:/home/azureuser/Phscarper/
else
...
ssh -i ~/Downloads/phscraper_key.pem azureuser@20.193.130.170 << 'ENDSSH'
cd ~/Phscarper
